 ...
 * zach@as3ics.com
 *
 * Entry point into launching the backend.  Use 'npm start' to launch
 */
...
mongoose.set("debug", (collectionName, method, query, doc) => {
    winston.info(`${collectionName}.${method}`, JSON.stringify(query), doc);
});
...
app.use(function (req, res, next) {
    if (app.config.server.loggingEnabled == true) {
        winston.info(Date() + "\t\t" + req.method + " " + req.url);
...
var revision, branch, date = undefined;
exec("git rev-parse HEAD", function (err, res) { if (!err) { revision = res.toString().trim(); } });
exec("git rev-parse --abbrev-ref HEAD", function (err, res) { if (!err) { branch = res.toString().trim(); } });
exec("git log -1 --format=%cd", function (err, res) { if (!err) { date = new Date(res.toString()); } });
...
module.exports.listen = function (done) {
    ...
    */
    ...
    var corsOptions = {
        ...
        allowedHeaders: "Authorization,DNT,X-CustomHeader,Keep-Alive,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type",
