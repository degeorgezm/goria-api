...
const { ApiError, client2: square, client2 } = require('../square/square.client');
...
function retrieveProducts(req, list, done) {
    ...
    async.waterfall(
        functions
        , function (err, items, products, counter) {
            if (err) {
                return done({ status: 500, body: { error: err } });
            } else {
...
function splitItemInOrder(req, order, returnId, done) {
    ...
    if (parseInt(item.quantity) != parseInt(req.body.quantity)) {
        if (parseInt(req.body.quantity) > parseInt(item.quantity)) {
            return (done({ status: 500, body: { error: "Can't return more quantity than there is of item quantity" } }));
        }
        ...
        order.items.id(req.params.itemId).quantity = parseInt(item.quantity) - parseInt(req.body.quantity);
        order.save();
    ...
    User.findOne(query, projection)
        .exec(function (err, user) {
            if (err) {
                ...
                var text = "You are receiving this message because you have recently requested a new return request from Designica. This email is to confirm that your request has been received and that it will be reviewed within the next two business days for approval. If approved you will receive an additional message with instruction on how to proceed with your return. \n\n";
                text += "For your reference your return id is: " + returnId + "\n\n";
                text += "If you change your mind and no longer wish to return this item you can cancel your return request by visiting the Dashboard from your Designica account. Please do not respond to this email, it is not monitored and it will not be checked. If you have any questions or concerns please use our standard customer support channels. Thank you.";

                ejs.renderFile('app/email_templates/return_request_requested.ejs', { user: user.firstName, returnId: returnId }, function (err, htmlString) {
                    ...
                    mailgun.messages().send(data, function (err, body) {
                        if (err) { done({ status: 500, body: { error: err } }); }
                        return done(null, req, order, returnId);
                    });
...
function updateItemStatussProcessing(req, order, done) {
    ...
    functions.push(async.apply(updateItemStatusProcessing, req, order, counter, now));
    for (let i = 1; i < array.length; i++) {
        functions.push(updateItemStatusProcessing);
    }

    async.waterfall(
        functions, function (err, req, array, counter, now) {
            if (err) {
                return done({ status: 500, body: { error: err } });
            } else {
                return done(null, req, order);
...
function retrieveOrderForUpdateStatus(req, done) {
    ...
    Order.findOne(query)
        .deepPopulate('user items.product items.product.variants items.product.sizes items.product.type items.product.department items.product.groups items.product.image items.product.images items.size items.variant items.promotions items.product.stock.variant status.user items.status.user items.return_request.user')
        .exec(function (err, order) {
            if (err) {
                console.log(err);
                return done({ status: 500, body: { error: err } });
...
function updateItemStatussRefunded(req, order, done) {
    ...
    functions.push(async.apply(updateItemStatusRefunded, req, order, counter, now));
    for (let i = 1; i < array.length; i++) {
...
function updateOrderStatusDropOrder(req, order, done) {
    ...
    Order.update(query, { $push: { status: status } })
        .exec(function (err) {
            if (err) {
                console.log(err);
                return done({ status: 500, body: { error: err } });
...
exports.add_to_wishlist = function (req, res) {
    ...
    Wishlist.update(query, { $push: { items: stock } })
        .exec(function (err) {
            if (err) {
...
exports.add_to_bag = function (req, res) {
    ...
    Bag.update(query, { $push: { items: stock } })
        .exec(function (err) {
            if (err) {
                return res.status(500).send(err);
            } else {
...
exports.remove_from_wishlist = function (req, res) {
    ...
    Wishlist.update(query, { $pull: { items: { _id: req.params.stockId } } })
        .exec(function (err) {
            if (err) {
                return res.status(500).send(err);
            } else {
...
exports.remove_from_bag = function (req, res) {
    ...
    Bag.update(query, { $pull: { items: { _id: req.params.stockId } } })
        .exec(function (err) {
            if (err) {
                return res.status(500).send(err);
            } else {
...
exports.request_orders_by_item_status = function (req, res) {
    ...
    if (req.body.status == "all") {
        query['items.status.status'] = { $in: ['Return Requested', 'Return Approved', 'Return Denied', 'Return Shipped', 'Return Received', 'Return Processing', 'Returned'] }
    } else {
...
exports.drop_order = function (req, res) {
    ...
    async.waterfall([
        ...
        if (err) {
            ...
            try {
                const response = await client2.refundsApi.refundPayment(
                    refund
                );
                ...
                Order.update(query, { $push: { refund_stubs: _refund } })
                    .exec(function (err) {
                        if (err) {
                            return res.status(500).send({ error: err });
                        } else {
...
exports.get_payment_refund_status = async function (req, res) {
    ...
    try {
        const response = await client2.refundsApi.getPaymentRefund(req.params.refundId);
