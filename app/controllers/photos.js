...
var uploadS3User = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            cb(null, global.S3_IMG_HANDLER_FOLDER + '/users/' + req.params.userId + '/' + file.fieldname + '-' + Date.now() + '.' + 'jpeg');
        },
...
var uploadS3Product = multer({
    storage: multerS3({
        ...
        // Set key/ filename as original uploaded name
        key: function (req, file, cb) {
            cb(null, global.S3_IMG_HANDLER_FOLDER + '/products/' + req.params.productId + '/' + file.fieldname + '-' + Date.now() + '.' + 'jpeg');
        },
    }),
    fileFilter: fileFilterPicture,
    limits: { fileSize: config.server.max_file_size }
...
function removeFolderFromFilenameProduct(req) {
    var X = req.file.key;
    var Y = global.S3_IMG_HANDLER_FOLDER + '/products/' + req.params.productId + '/';
    return X.slice(X.indexOf(Y) + Y.length);
}
...
function deleteProductPhotoS3(photo, product, done) {
    ...
    params = {
        Bucket: global.S3_BUCKET_NAME,
        Key: global.S3_IMG_HANDLER_FOLDER + '/products/' + product._id + '/' + photo.filename
    };
                    ...
                    Photo
                        ...
                        .exec(function (err, photo) {
                            if (err) {
                                ...
                                params = {
                                    Bucket: global.S3_BUCKET_NAME,
                                    Key: global.S3_IMG_HANDLER_FOLDER + '/users/' + req.params.userId + '/' + photo.filename
                                };
...
function uploadPhotoS3User(req, res, done) {
    uploadS3User.single('photo')(req, res, function (err) { //upload the file in the fieldname "photo"
...
function uploadPhotoS3Product(req, res, done) {
    uploadS3Product.single('photo')(req, res, function (err) { //upload the file in the fieldname "photo"
        if (err || req.file == undefined) {
            return done({ status: 500, body: { error: err } });
        } else { return done(null, req); }
    });
                ...
                Photo.findOne(query)
                    .exec(function (err, photo) {
                        if (err) {
                            ...
                            var params = {
                                Bucket: global.S3_BUCKET_NAME,
                                Key: global.S3_IMG_HANDLER_FOLDER + '/products/' + req.params.productId + '/' + photo.filename
                            };
    ...
    async.waterfall(
        ...
        function (err, photo, product) {
            if (err) {
                ...
                Product.update(query, { $pull: { images: { $in: [photo._id] } } })
                    .exec(function (err) {
                        if (err) {
                            return res.status(500).send({ error: err });
                        } else {
...
exports.set_photo_primary = function (req, res) {
    ...
    Product.update(query, { image: req.params.photoId })
        .exec(function (err) {
            if (err) {
                return res.status(500).send({ error: err });
            } else {
