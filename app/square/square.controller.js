...
function calculateDiscount(items, array, counter, done) {
    ...
    product.sales.forEach(element => {
        if (new Date() >= new Date(element.start) && (new Date() <= new Date(element.end))) {
    ...
    if ((product.promotions && product.promotions.length > 0) || (active_sales && active_sales.length > 0)) {

        // First Find Stackable Discount Amount
        let stackablePromotions = product.promotions ? product.promotions.filter(element => ((element.stackable == true) && (element.minimum == 0))) : [];
        let stackableSales = active_sales ? active_sales.filter(element => (element.stackable == true)) : [];
        ...
        // First Find Stackable Discount Amount
        let standalonePromotions = product.promotions ? product.promotions.filter(element => (element.stackable == false)) : [];
        let standaloneSales = active_sales ? product.sales.filter(element => (element.stackable == false)) : [];
...
function calculateTotals(req, bag, products, done) {
    ...
    if (bag.items.length != products.length) {
        return done({ status: 500, body: { error: "Calculate Totals: Array size mismatch" } })
    }
...
function compareTotals(req, bag, products, content, done) {
    ...
    if (valid == false) {
        return done({ status: 500, body: { error: "Compare Totals: Value mismatch" } })
    }
    ...
    Macros.findOne({ name: global.MACROS_DB_NAME })
        .exec(function (err, macros) {
            if (err) {
                ...
                return done(null, req, bag, products, content, statusCode, result, orderId);
            }
        });
}

function updateMacrosCount(req, bag, products, content, statusCode, result, orderId, done) {
...
function createOrder(req, bag, products, content, statusCode, result, orderId, done) {
    ...
    for (let i = 0; i < bag.items.length; i++) {
        ...
        let item = OrderItem({
            ...
            status: status,
            tax: parseFloat((parseFloat((products[i].price - products[i].discount).toFixed(2)) * SalesTable[req.body.shipping_address.state]).toFixed(2)),
            list_price: products[i].price,
            discount: products[i].discount,
            discount_price: parseFloat((products[i].price - products[i].discount).toFixed(2)),
...
function clearBag(req, bag, products, content, statusCode, result, order, done) {
    ...
    Bag.update(query, { $pull: { items: { _id: _in } } })
        .exec(function (err) {
            if (err) {
                return done({ status: 500, body: { error: err } });
            } else {
        ...
        if (err) {
            ...
            await retry(async (bail, attempt) => {
                try {
                    ...
                    async.waterfall([
                        async.apply(getOrderId, req, bag, products, content, statusCode, toObject(result)),
                        updateMacrosCount,
                        createOrder,
                        clearBag
                    ], async function (err, reqOut, bag, products, content, statusCode, result, order) {
                        if (err) {
                            console.log(err);
                            return res.status(err.status).send(err.body);
                        } else {
                    ...
                    if (ex instanceof ApiError) {
                        ...
                        logger.info(`Error creating payment on attempt ${attempt}: ${ex}`);
