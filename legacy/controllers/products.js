...
function getVariantShortcodes(req, object, done) {
  ...
  Variant.find({ _id: { $exists: true } }).exec(function (err, variants) {
    if (err) {
...
function getSizeShortcodes(req, object, done) {
  ...
  Size.find({ _id: { $exists: true } }).exec(function (err, sizes) {
    if (err) {
      return done({ status: 500, body: { error: err } });
    } else {
...
function getDepartmentShortcodes(req, object, done) {
  Department.findOne({ _id: req.body["department"] }).exec(function (
    err,
    department
  ) {
    if (err) {
...
function getTypeShortcodes(req, object, done) {
  Type.findOne({ _id: req.body["type"] }).exec(function (err, type) {
    if (err) {
      return done({ status: 500, body: { error: err } });
    } else {
...
function getBrandShortcodes(req, object, done) {
  Brand.findOne({ _id: req.body["brand"] }).exec(function (err, brand) {
    if (err) {
      return done({ status: 500, body: { error: err } });
    } else {
...
function retrieveProduct(req, promotions, done) {
  ...
  Product.findOne(query)
    .deepPopulate(
      "image type department sizes variants stock.variant stock.size brand groups images price_changes.user sales.user"
    )
...
function retrieveLeanProducts(req, done) {
  ...
  if (req.query.department) {
    let departments = req.query.department
...
exports.create = function (req, res) {
  ...
  if (req.body["variants"].length != 0 && req.body["sizes"].length != 0) {
    ...
    return res
      .status(500)
      .send({ error: "Dimensions for product were not correct" });
  }
};
...
exports.update = function (req, res) {
  ...
  Product.update(query, req.body).exec(function (err, result) {
    if (err) {
      return res.status(500).send(err);
    } else {
...
exports.add_sale = function (req, res) {
  ...
  Product.update(query, { $push: { sales: req.body } }).exec(function (err) {
    if (err) {
      res.status(500).send(err);
    } else {
