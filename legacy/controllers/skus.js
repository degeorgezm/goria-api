...
function checkIfDepartmentNameExists(req, done) {
  ...
  Department.findOne(query).exec(function (err, department) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a department with this name already exists" },
      });
    }
  });
}
...
function checkIfDepartmentSKUExists(req, done) {
  ...
  Department.findOne(query).exec(function (err, department) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a department with this sku already exists" },
      });
    }
  });
}
...
function checkIfTypeNameExists(req, done) {
  ...
  Type.findOne(query).exec(function (err, type) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a type with this name already exists" },
      });
    }
  });
}
...
function checkIfTypeSKUExists(req, done) {
  ...
  Type.findOne(query).exec(function (err, type) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a type with this sku already exists" },
      });
    }
  });
}
...
function checkIfSizeNameExists(req, done) {
  ...
  Size.findOne(query).exec(function (err, size) {
    if (err) {
      return done({ status: 500, body: { error: err } });
    } else if (!size) {
      return done(null, req);
      ...
      return done({
        status: 409,
        body: { error: "a size with this name already exists" },
      });
    }
  });
}
...
function checkIfSizeSKUExists(req, done) {
  ...
  Size.findOne(query).exec(function (err, size) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a size with this sku already exists" },
      });
    }
  });
}
...
function checkIfBrandNameExists(req, done) {
  ...
  Brand.findOne(query).exec(function (err, brand) {
    if (err) {
      return done({ status: 500, body: { error: err } });
    } else if (!brand) {
      return done(null, req);
      ...
      return done({
        status: 409,
        body: { error: "a brand with this name already exists" },
      });
    }
  });
}
...
function checkIfBrandSKUExists(req, done) {
  ...
  Brand.findOne(query).exec(function (err, brand) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a brand with this sku already exists" },
      });
    }
  });
}
...
function checkIfVariantNameExists(req, done) {
  ...
  Variant.findOne(query).exec(function (err, variant) {
    if (err) {
      return done({ status: 500, body: { error: err } });
    } else if (!variant) {
      return done(null, req);
      ...
      return done({
        status: 409,
        body: { error: "a variant with this name already exists" },
      });
    }
  });
}
...
function checkIfVariantSKUExists(req, done) {
  ...
  Variant.findOne(query).exec(function (err, variant) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a variant with this sku already exists" },
      });
    }
  });
}
...
function checkIfGroupNameExists(req, done) {
  ...
  Group.findOne(query).exec(function (err, group) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a group with this name already exists" },
      });
    }
  });
}
...
exports.create_department = function (req, res) {
  ...
  async.waterfall(
    [async.apply(checkIfDepartmentNameExists, req), checkIfDepartmentSKUExists],
    function (err) {
      if (err) {
...
exports.update_department = function (req, res) {
  ...
  // Update Department
  Department.update(query, req.body).exec(function (err, result) {
    if (err) {
      return res.status(500).send(err);
    } else {
...
exports.create_type = function (req, res) {
  ...
  async.waterfall(
    [async.apply(checkIfTypeNameExists, req), checkIfTypeSKUExists],
    function (err) {
      if (err) {
        return res.status(err.status).send(err.body);
      } else {
...
exports.update_type = function (req, res) {
  ...
  // Update Type
  Type.update(query, req.body).exec(function (err, result) {
    if (err) {
      return res.status(500).send(err);
    } else {
      return res.status(200).send(result);
...
exports.create_size = function (req, res) {
  ...
  async.waterfall(
    [async.apply(checkIfSizeNameExists, req), checkIfSizeSKUExists],
    function (err) {
      if (err) {
        return res.status(err.status).send(err.body);
      } else {
...
exports.update_size = function (req, res) {
  ...
  // Update Size
  Size.update(query, req.body).exec(function (err, result) {
    if (err) {
      return res.status(500).send(err);
    } else {
      return res.status(200).send(result);
...
exports.create_brand = function (req, res) {
  ...
  async.waterfall(
    [async.apply(checkIfBrandNameExists, req), checkIfBrandSKUExists],
    function (err) {
      if (err) {
        return res.status(err.status).send(err.body);
      } else {
...
exports.update_brand = function (req, res) {
  ...
  // Update Brand
  Brand.update(query, req.body).exec(function (err, result) {
    if (err) {
      return res.status(500).send(err);
    } else {
      return res.status(200).send(result);
...
exports.create_group = function (req, res) {
  ...
  async.waterfall([async.apply(checkIfGroupNameExists, req)], function (err) {
    if (err) {
      return res.status(err.status).send(err.body);
    } else {
...
exports.update_group = function (req, res) {
  ...
  // Update Group
  Group.update(query, req.body).exec(function (err, result) {
    if (err) {
      return res.status(500).send(err);
    } else {
      return res.status(200).send(result);
...
exports.create_variant = function (req, res) {
  ...
  async.waterfall(
    [async.apply(checkIfVariantNameExists, req), checkIfVariantSKUExists],
    function (err) {
      if (err) {
        return res.status(err.status).send(err.body);
      } else {
...
exports.update_variant = function (req, res) {
  ...
  // Update Variant
  Variant.update(query, req.body).exec(function (err, result) {
    if (err) {
      return res.status(500).send(err);
    } else {
      return res.status(200).send(result);
