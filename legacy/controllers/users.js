...
function checkIfPhoneExists(req, done) {
  ...
  User.findOne(query).exec(function (err, user) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a user with this phone number already exists" },
      });
    }
  });
}
...
function checkIfUsernameExists(req, done) {
  ...
  User.findOne(query).exec(function (err, user) {
    if (err) {
      ...
      return done({
        status: 409,
        body: { error: "a user with username already exists" },
      });
    }
  });
}
    ...
    function (err) {
      if (err) {
        ...
        user.birthday = new Date(
          "<1970-" + req.body.month + "-" + req.body.day + ">"
        );
        ...
        user.save(function (err, usr) {
          if (err) {
            ...
            async.waterfall(
              [async.apply(bagCheck, usr), wishlistCheck],
              function (err) {
                var text =
                  "Welcome to Designica! You are receiving this message because you have recently created a new account on Designica.\n\n";
                text +=
                  "This email is to confirm that your account has been created with the username " +
                  usr.username +
                  " and that you may be receiving occasional messages in the future at this address based on your activity on Designica.\n\n";
                text +=
                  "Thank you from the Designica Team for creating an account, and we are looking forward to working with you in the future!";

                ejs.renderFile(
                  "app/email_templates/account_created.ejs",
                  { user: usr.firstName, username: usr.username },
...
exports.delete = function (req, res) {
  ...
  User.remove(query).exec(function (err) {
    if (err) {
    ...
    if (err) {
      ...
      user.isValidPassword(req.body.previous, function (err, isValid) {
        if (err) {
          ...
          return res
            .status(401)
            .send({ error: "Password error. Not authorized." });
        } else {
  ...
  User.findOne(query, function (err, user) {
    ...
    user.save(function (err) {
      if (err) {
        ...
        var text =
          "You are receiving this because you (or someone else) have requested to reset the password for your account. If you did not request this, please ignore this email and your password will remain unchanged. Please click the link below to reset your password.";

        var link =
          global.APP_BASE_URL +
          global.APP_PASSWORD_RESET_URL +
          "?email=" +
          user.email +
          "&token=" +
          user._resetPasswordToken;

        ejs.renderFile(
          "app/email_templates/password_reset_request.ejs",
          { user: user.firstName, password_reset_link: link },
          function (err, htmlString) {
            if (err) {
              console.error("ejs error", err);
            }
  ...
  User.findOne(query, function (err, user) {
    ...
    user.save(function (err) {
      if (err) {
        ...
        var text =
          "This is a confirmation that your password has been changed. If you did not make this change, please contact us immediately at support@designicajewelry.com";
        var link =
          global.APP_BASE_URL +
          global.APP_PASSWORD_RESET_URL +
          "?email=" +
        ...
        ejs.renderFile(
          "app/email_templates/password_reset_success.ejs",
          { user: user.firstName },
          function (err, htmlString) {
            if (err) {
              console.error("ejs error", err);
