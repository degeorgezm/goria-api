...
module.exports.set = function (fs, winston, app) {
  ...
  User.find(query).exec(function (err, users) {
    if (err) {
      ...
      user.twilio =
        "1" + process.env.DEFAULT_ADMIN_PHONE.replace(/[^0-9]/g, "");
  ...
  Variant.findOne({ sku_shortcode: "NV" }).exec(function (err, variant) {
    if (err) {
  ...
  Macros.findOne({ name: global.MACROS_DOC_NAME }).exec(function (err, macros) {
    if (err) {
      ...
      var macros = Macros();
      macros.name = global.MACROS_DOC_NAME;
      macros.count = 0;
  ...
  Group.findOne({ name: global.DEFAULT_GROUP_NAME }).exec(function (
    err,
    group
  ) {
    if (err) {
  ...
  Setting.findOne({ name: global.DEFAULT_SETTINGS_DOC_NAME }).exec(function (
    err,
    setting
  ) {
    if (err) {
      ...
      var setting = Setting();
      setting.name = global.DEFAULT_SETTINGS_DOC_NAME;
